...
/**
 * 체결이 실제로 일어난 거래만 기록하는 append-only 원장. UPDATE/DELETE 금지 (CLAUDE.md 절대 규칙).
 * 체결 전 주문(지정가 대기)은 이 테이블에 나타나지 않는다 — {@link PendingOrder} 참고.
 * 체결 전 취소는 PENDING_ORDERS 행 자체를 CANCELLED로 바꾸는 것이라 이 원장에 아무 기록도 남기지 않는다.
 */
@Getter
